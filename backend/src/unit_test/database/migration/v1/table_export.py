...
from backend.src.unit_test.database.social_media_stream_database              import   SocialMediaStreamDataBase
from backend.src.unit_test.database.migration.v1.live                         import   LiveRecordTable
from backend.src.unit_test.database.migration.v1.room                         import   RoomAttributeTable,                                  \
                                                                                       ...
                                                                                       RoomVsRoleTable
from backend.src.unit_test.database.migration.v1.source                       import   BadgeImageTable,                                     \
                                                                                       PayGradeIconTable,                                   \
...
from backend.src.unit_test.database.migration.v1.stream                       import   LiveStreamTable,                                     \
...
from backend.src.unit_test.database.migration.v1.user                         import   RoomOwnerTable,                                      \
                                                                                       OwnRoomFlagTable,                                    \
                                                                                       OwnRoomIdTable,                                      \
                                                                                       FansClubTable,                                       \
...
def safe_json_loads(value, default=None):
  """Safely parse JSON-like fields from DB.

  - If value is None => return default or {}
  - If value is already a dict/list => return it
  - If value is a str/bytes => try json.loads, on failure return default or original string
  """
  if value is None:
...
def safe_timestamp(value, default=0, ms=False):
  try:
    if value is None:
      return default
    if hasattr(value, 'timestamp'):
      t = value.timestamp()
      return int(t * 1000) if ms else int(floor(t))
    if isinstance(value, (int, float)):
      return int(value * 1000) if ms else int(floor(value))
    if isinstance(value, str):
      v = value.strip()
      if v == "":
        return default
      f = float(v)
      return int(f * 1000) if ms else int(floor(f))
    return default
  except Exception:
    return default


def safe_index(value, size, default=-1):
  index = safe_int(value, default=default)
...
def export_live_data(db:SocialMediaStreamDataBase, identifier:dict = None, output_path:str = None):
  ...
  try:
    now           = get_dict_attr(identifier, "$.now")
    ...
    start_time    = get_dict_attr(identifier, "$.start_time")
  ...
  try:
    ...
    room_deco_list                  = [dict() for _ in room_deco_tuple_list]
    room_deco_input_rect_list       = [[] for _ in room_deco_tuple_list]
    room_deco_reservation_list      = [dict() for _ in room_deco_tuple_list]
    room_deco_text_foot_config_list = [dict() for _ in room_deco_tuple_list]
    for room_deco in room_deco_tuple_list:
      deco_index = safe_index(get_dict_attr(room_deco, "$.deco_index"), len(room_deco_list))
      room_deco_dict = dict()
      ...
      room_deco_list[deco_index] = room_deco_dict
      ...
      try:
        ...
        deco_input_list = [None for _ in room_deco_input_rect_tuple_list]
        for room_deco_input_rect in room_deco_input_rect_tuple_list:
          input_rect       = get_dict_attr(room_deco_input_rect, "$.input_rect")
          input_rect_index = safe_index(get_dict_attr(room_deco_input_rect, "$.input_rect_index"), len(deco_input_list))
          deco_input_list[input_rect_index] = input_rect
        room_deco_input_rect_list[deco_index] = deco_input_list
      except Exception as e:
      ...
      try:
        ...
        if len(room_deco_reservation_tuple_list) != 0:
          ...
          room_deco_reservation_list[deco_index] = room_deco_reservation
      except Exception as e:
      ...
      try:
        ...
        if len(room_deco_text_foot_config_tuple_list) != 0:
          ...
          room_deco_text_foot_config_list[deco_index] = room_deco_text_foot_config
      except Exception as e:
  ...
  if len(room_deco_list) != 0:
    ...
    """
    deco_image_avg_color_list    = ['' for _ in room_deco_list]
    deco_image_flex_setting_list = [[] for _ in room_deco_list]
    deco_image_height_list       = [0 for _ in room_deco_list]
    deco_image_image_type_list   = [0 for _ in room_deco_list]
    deco_image_is_animated_list  = [False for _ in room_deco_list]
    deco_image_open_web_url_list = ['' for _ in room_deco_list]
    deco_image_text_setting_list = [[] for _ in room_deco_list]
    deco_image_uri_list          = ['' for _ in room_deco_list]
    deco_image_url_list          = [[] for _ in room_deco_list]
    deco_image_width_list        = [0 for _ in room_deco_list]
    
    ##
    ## PictureTable
    ##
    deco_image_exist_list = [False for _ in room_deco_list]
    image_prefix   = 'image'
    ...
    try:
      ...
      for deco_image_pic_tuple in deco_image_pic_tuple_list:
        ...
        if str(deco_image_pic_label).startswith(image_prefix):
          deco_index = safe_index(deco_image_pic_label[len(image_prefix):], len(room_deco_list))

          deco_image_avg_color_list[deco_index] = deco_image_pic_tuple.get("avg_color", '')
          deco_image_height_list[deco_index] = deco_image_pic_tuple.get("height", 0)
          deco_image_image_type_list[deco_index] = deco_image_pic_tuple.get("image_type", 0)
          deco_image_is_animated_list[deco_index] = bool(deco_image_pic_tuple.get("is_animated", False))
          deco_image_open_web_url_list[deco_index] = deco_image_pic_tuple.get("open_web_url", '')
          deco_image_uri_list[deco_index] = deco_image_pic_tuple.get("uri", '')
          deco_image_width_list[deco_index] = deco_image_pic_tuple.get("width", 0)
          deco_image_exist_list[deco_index] = True
    except Exception as e:
      get_logger().error(f"{e}: {room_deco_table.get_name()} >> >> >> >> data.room.deco_list[{deco_index}].image")
      deco_image_avg_color_list[deco_index] = ''
      deco_image_height_list[deco_index] = 0
      deco_image_image_type_list[deco_index] = 0
      deco_image_is_animated_list[deco_index] = False
      deco_image_open_web_url_list[deco_index] = ''
      deco_image_uri_list[deco_index] = ''
      deco_image_width_list[deco_index] = 0
      deco_image_exist_list[deco_index] = False
      raise e
    ...
    for deco_index in range(0, len(room_deco_list)):
      if deco_image_exist_list[deco_index] is True:
        ...
        try:
          ...
          deco_image_flex_setting_list[deco_index] = deco_image_flex_setting
        except Exception as e:
        ...
        try:
          ...
          deco_image_text_setting_list[deco_index] = single_deco_image_pic_text_setting
        except Exception as e:
        ...
        try:
          ...
          deco_image_url_list[deco_index] = deco_image_pic_url_list
        except Exception as e:
    ...
    """
    deco_nine_patch_image_avg_color_list    = ['' for _ in room_deco_list]
    deco_nine_patch_image_flex_setting_list = [[] for _ in room_deco_list]
    deco_nine_patch_image_height_list       = [0 for _ in room_deco_list]
    deco_nine_patch_image_image_type_list   = [0 for _ in room_deco_list]
    deco_nine_patch_image_is_animated_list  = [False for _ in room_deco_list]
    deco_nine_patch_image_open_web_url_list = ['' for _ in room_deco_list]
    deco_nine_patch_image_text_setting_list = [[] for _ in room_deco_list]
    deco_nine_patch_image_uri_list          = ['' for _ in room_deco_list]
    deco_nine_patch_image_url_list          = [[] for _ in room_deco_list]
    deco_nine_patch_image_width_list        = [0 for _ in room_deco_list]
    ...
    try:
      ...
      for deco_nine_patch_image_pic_tuple in deco_nine_patch_image_pic_tuple_list:
        ...
        if str(deco_nine_patch_image_pic_label).startswith(image_prefix):
          deco_index = safe_index(deco_nine_patch_image_pic_label[len(image_prefix):], len(room_deco_list))

          deco_nine_patch_image_avg_color_list[deco_index] = deco_nine_patch_image_pic_tuple.get("avg_color", '')
          deco_nine_patch_image_height_list[deco_index] = deco_nine_patch_image_pic_tuple.get("height", 0)
          deco_nine_patch_image_image_type_list[deco_index] = deco_nine_patch_image_pic_tuple.get("image_type", 0)
          deco_nine_patch_image_is_animated_list[deco_index] = bool(deco_nine_patch_image_pic_tuple.get("is_animated", False))
          deco_nine_patch_image_open_web_url_list[deco_index] = deco_nine_patch_image_pic_tuple.get("open_web_url", '')
          deco_nine_patch_image_uri_list[deco_index] = deco_nine_patch_image_pic_tuple.get("uri", '')
          deco_nine_patch_image_width_list[deco_index] = deco_nine_patch_image_pic_tuple.get("width", 0)
          nine_patch_image_exist_list[deco_index] = True
    except Exception as e:
      get_logger().error(f"{e}: {room_deco_table.get_name()} >> >> >> >> data.room.deco_list[{deco_index}].image")
      deco_nine_patch_image_avg_color_list[deco_index] = ''
      deco_nine_patch_image_height_list[deco_index] = 0
      deco_nine_patch_image_image_type_list[deco_index] = 0
      deco_nine_patch_image_is_animated_list[deco_index] = False
      deco_nine_patch_image_open_web_url_list[deco_index] = ''
      deco_nine_patch_image_uri_list[deco_index] = ''
      deco_nine_patch_image_width_list[deco_index] = 0
    ...
    for deco_index in range(0, len(room_deco_list)):
      if len(nine_patch_image_exist_list) != 0 and nine_patch_image_exist_list[deco_index] is True:
        ...
        try:
          ...
          deco_nine_patch_image_flex_setting_list[deco_index] = deco_nine_patch_image_flex_setting
        except Exception as e:
        ...
        try:
          ...
          deco_nine_patch_image_text_setting_list[deco_index] = single_deco_nine_patch_image_pic_text_setting
        ...
        try:
          ...
          deco_nine_patch_image_url_list[deco_index] = deco_nine_patch_image_pic_url_list
        except Exception as e:
    ...
    try:
      ...
      if room_link_mic_battle_score_tuple_list:
        for battle_score_tuple in room_link_mic_battle_score_tuple_list:
          ...
          set_dict_attr(room_link_mic_battle_score_dict, "$.user_id", safe_int(get_dict_attr(battle_score_tuple, "$.user_id")), force=True)
  ...
  for badge_image_index in range(0, len(owner_badge_image_v2_uri_list)):
    ...
    try:
      ...
      ##
      ## PictureContentTable >> data.room.owner.badge_image_list_v2.content
      ##
  ...
  if own_room_exist_flag is True:
    ...
    try:
      ...
      for owner_own_room_id in owner_own_room_id_tuple_list:
        owner_own_room_id_list.insert(safe_int(get_dict_attr(owner_own_room_id, "$.room_id_index")), safe_int(get_dict_attr(owner_own_room_id, "$.room_id")))
    except Exception as e:
  ...
  try:
    ...
    short_touch_type_list = sorted(short_touch_type_list, key=lambda v: str(v))
  except Exception as e:
  ...
  try:
    ...
    for short_touch_type in short_touch_type_list:
      ...
      room_temp_state_strategy_map_tuple_list = sorted(
        room_temp_state_strategy_map_tuple_list,
        key=lambda v: str(v.get('strategy_type', ''))
      )
  ...
  """
  room = dict()
  set_dict_attr(room, "$.AnchorABMap",                          safe_json_loads(room_attribute_tuple.get('AnchorABMap'), {}),    force=True)
  ...
  set_dict_attr(room, "$.app_id",                               safe_int(room_attribute_tuple.get('app_id')),                      force=True)
  ...
  set_dict_attr(room, "$.client_version",                       safe_int(room_attribute_tuple.get('client_version')),              force=True)
  ...
  set_dict_attr(room, "$.create_time",                          safe_timestamp(room_attribute_tuple.get('create_time'), default=0),           force=True)
  ...
  if len(room_deco_list) != 0:
    ...
    for deco_index in range(0, len(room_deco_list)):
      ...
      set_dict_attr(room_deco_dict, "$.reservation.appointment_id", safe_int(get_dict_attr(room_deco_reservation_list[deco_index], '$.appointment_id')),  force=True)
      ...
      set_dict_attr(room_deco_dict, "$.reservation.room_id",        safe_int(get_dict_attr(room_deco_reservation_list[deco_index], '$.room_id')),         force=True)
      ...
      if len(room_deco_text_foot_config_list) > deco_index:
        ...
        set_dict_attr(room_deco_dict, "$.text_font_config.FontID",      safe_int(get_dict_attr(room_deco_text_foot_config_list[deco_index], '$.FontID')),      force=True)
  ...
  """
  set_dict_attr(room, "$.dynamic_cover_dict",                   safe_json_loads(room_attribute_tuple.get('dynamic_cover_dict'), {}),   force=True)
  ...
  set_dict_attr(room, "$.extra.xigua_uid",                      safe_int(room_attribute_tuple.get('xigua_uid')),                         force=True)
  ...
  set_dict_attr(room, "$.fcdn_appid",                           safe_int(room_attribute_tuple.get('fcdn_appid')),                        force=True)
  ...
  set_dict_attr(room, "$.finish_time",      safe_timestamp(room_attribute_tuple.get('finish_time'), default=0),       force=True)
  ...
  set_dict_attr(room, "$.group_id",         safe_int(room_attribute_tuple.get('group_id')),          force=True)
  ...
  set_dict_attr(room, "$.id",                  safe_int(room_attribute_tuple.get('id')),                                force=True)
  ...
  if room_record_tuple.get('last_ping_time', 0) is None:
    ...
    set_dict_attr(room, "$.last_ping_time",           safe_timestamp(room_record_tuple.get('last_ping_time'), default=0),                        force=True)
  ...
  if room_link_mic_tuple:
    ...
    if room_link_mic_battle_setting_tuple:
      ...
      set_dict_attr(room, "$.link_mic.battle_settings.battle_id",     safe_int(get_dict_attr(room_link_mic_battle_setting_tuple, "$.battle_id")),                          force=True)
      set_dict_attr(room, "$.link_mic.battle_settings.channel_id",    safe_int(get_dict_attr(room_link_mic_battle_setting_tuple, "$.channel_id")),                         force=True)
      ...
      set_dict_attr(room, "$.link_mic.battle_settings.start_time",    safe_timestamp(get_dict_attr(room_link_mic_battle_setting_tuple, "$.start_time"), default=0),           force=True)
      set_dict_attr(room, "$.link_mic.battle_settings.start_time_ms", safe_timestamp(get_dict_attr(room_link_mic_battle_setting_tuple, "$.start_time_ms"), default=0, ms=True), force=True)
    ...
    set_dict_attr(room, "$.link_mic.rival_anchor_id",      safe_int(get_dict_attr(room_link_mic_tuple, "$.rival_anchor_id")), force=True)
  ...
  """
  set_dict_attr(room, "$.linker_map",               safe_json_loads(room_record_tuple.get('linker_map'), {}),         force=True)
  ...
  set_dict_attr(room, "$.live_id",               safe_int(room_attribute_tuple.get('live_id')),                         force=True)
  ...
  set_dict_attr(room, "$.living_room_attrs.room_id",       safe_int(room_record_tuple.get('id')),            force=True)
  ...
  set_dict_attr(room, "$.official_channel_uid",     safe_int(room_attribute_tuple.get('official_channel_uid')),      force=True)
  ...
  """
  if bool(room_owner_auth_info_tuple.get('exist_authentication_info', False)) is True:
    set_dict_attr(room, "$.owner.authentication_info.account_cert_info",                   safe_json_loads(room_owner_auth_info_tuple.get('account_cert_info'), {}),     force=True)
    set_dict_attr(room, "$.owner.authentication_info.account_type_info.account_type_map",  safe_json_loads(room_owner_auth_info_tuple.get('account_type_map'), {}),      force=True)
  ...
  """
  set_dict_attr(room, "$.owner.fans_club.data.anchor_id",               safe_int(owner_fans_club_tuple.get('anchor_id')),  force=True)
  ...
  set_dict_attr(room, "$.owner.fans_club.prefer_data",                safe_json_loads(owner_fans_club_tuple.get('prefer_data'),          {}),          force=True)
  ...
  set_dict_attr(room, "$.owner.id",                               safe_int(room_owner_tuple.get('id')),                        force=True)
  ...
  if room_owner_tuple.get('modify_time', 0) is not None:
    set_dict_attr(room, "$.owner.modify_time",                      safe_timestamp(room_owner_tuple.get('modify_time'), default=0), force=True)
  else:
  ...
  set_dict_attr(room, "$.owner.short_id",         safe_int(room_owner_tuple.get('short_id')),         force=True)
  ...
  set_dict_attr(room, "$.owner_device_id", safe_int(room_attribute_tuple.get('owner_device_id')), force=True)
  set_dict_attr(room, "$.owner_open_id",   room_attribute_tuple.get('owner_open_id', ''),  force=True)
  set_dict_attr(room, "$.owner_user_id",   safe_int(room_attribute_tuple.get('owner_user_id')),   force=True)
  ...
  set_dict_attr(room, "$.pack_meta.extras",   safe_json_loads(pack_meta_tuple.get('extras'),   {}),   force=True)
  ...
  set_dict_attr(room, "$.room_view_stats.display_version",       safe_int(room_record_tuple.get('view_stats_display_version')),   force=True)
  ...
  set_dict_attr(room, "$.search_id",                             safe_int(room_attribute_tuple.get('search_id')),                 force=True)
  ...
  >> >> >> data.room.short_touch_area_config.elements
  """
  for element in sorted(room_short_touch_area_config_element_list, key=lambda v: str(v.get('type', ''))):
    set_dict_attr(room, f"$.short_touch_area_config.elements.{element.get('type', 0)}", element, force=True)

  set_dict_attr(room, "$.short_touch_area_config.forbidden_types_map",                             safe_json_loads(room_short_touch_area_config_tuple.get('forbidden_types_map'), {}),   force=True)
  ...
  >> >> >> >> data.room.short_touch_area_config.temp_state_condition_map
  """
  for temp_state_condition in sorted(
    room_temp_state_condition_map_list,
    key=lambda v: str(v.get('type', {}).get('strategy_type', ''))
  ):
    set_dict_attr(
      room,
      f"$.short_touch_area_config.temp_state_condition_map.{temp_state_condition.get('type', {}).get('strategy_type', 0)}",
  ...
  set_dict_attr(room, "$.start_time",                        safe_timestamp(room_attribute_tuple.get('start_time'), default=0),                    force=True)
  ...
  set_dict_attr(room, "$.stats.id",                                  safe_int(room_attribute_tuple.get('id')),                               force=True)
  if room_attribute_tuple.get('id', 0) == 0:
  ...
  set_dict_attr(room, "$.stream_id",         safe_int(room_record_tuple.get('stream_id')),                           force=True)
  ...
  >> >> >> >> data.room.stream_url.flv_pull_url
  """
  set_dict_attr(room, "$.stream_url.flv_pull_url", safe_json_loads(live_stream_tuple.get('flv_pull_url'), ''), force=True)

  """
  >> >> >> >> data.room.stream_url.flv_pull_url_params
  """
  set_dict_attr(room, "$.stream_url.flv_pull_url_params", safe_json_loads(live_stream_tuple.get('flv_pull_url_params'), {}), force=True)
  set_dict_attr(room, "$.stream_url.hls_pull_url", live_stream_tuple.get('hls_pull_url', ''), force=True)

  """
  >> >> >> >> data.room.stream_url.hls_pull_url_map
  """
  set_dict_attr(room, "$.stream_url.hls_pull_url_map", safe_json_loads(live_stream_tuple.get('hls_pull_url_map'), {}), force=True)
  set_dict_attr(room, "$.stream_url.hls_pull_url_params", safe_json_loads(live_stream_tuple.get('hls_pull_url_params'), ''), force=True)
  set_dict_attr(room, "$.stream_url.id", safe_int(live_stream_tuple.get('id')), force=True)
  ...
  """
  set_dict_attr(room, "$.stream_url.live_core_sdk_data.pull_data.hls_data_unencrypted", safe_json_loads(live_core_sdk_pull_data_tuple.get('hls_data_unencrypted'), {}), force=True)
  ...
  set_dict_attr(room, "$.stream_url.live_core_sdk_data.pull_data.version",               safe_int(live_core_sdk_pull_data_tuple.get('version')), force=True)
  ...
  >> >> >> >> data.room.stream_url.resolution_name
  """
  set_dict_attr(room, "$.stream_url.resolution_name",        safe_json_loads(live_stream_tuple.get('resolution_name'), {}),                force=True)

  set_dict_attr(room, "$.stream_url.rtmp_pull_url",          live_stream_tuple.get('rtmp_pull_url', ''),         force=True)
  set_dict_attr(room, "$.stream_url.rtmp_pull_url_params",   safe_json_loads(live_stream_tuple.get('rtmp_pull_url_params'), {}),  force=True)
  ...
  set_dict_attr(room, "$.vs_main_replay_id",             safe_int(room_attribute_tuple.get('vs_main_replay_id')),             force=True)
  ...
  set_dict_attr(room, "$.webcast_sdk_version",           safe_int(room_attribute_tuple.get('webcast_sdk_version')),           force=True)
  ...
  >>  extra
  """
  set_dict_attr(data,  "$.extra.now",                               safe_timestamp(now, default=0, ms=True),          force=True)
