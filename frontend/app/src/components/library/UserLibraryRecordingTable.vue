...
<template>
  <div class="table-scroll">
    <table class="table">
      ...
      <tbody>
        <tr
          v-for="row in recordings"
          :key="row.recording_id"
          :class="{ 'table__row--selected': row.recording_id === selectedId }"
        >
          <td>{{ creatorName(row.nickname) }}</td>
          <td class="table__title">{{ row.title ?? '—' }}</td>
          <td class="table__muted">{{ formatTimestamp(row.started_at) }}</td>
          <td class="table__muted">{{ formatTimestamp(row.finished_at) }}</td>
          <td class="table__muted">{{ formatTimestamp(row.created_at) }}</td>
          <td>
            <button type="button" class="table__view" @click="$emit('select', row.recording_id)">
              查看
            </button>
          </td>
        </tr>
